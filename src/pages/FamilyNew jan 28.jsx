...
const FamilyManagement = () => {
  ...
  const fetchParishes = async () => {
    try {
      ...
      // Fetch parishes with comprehensive configuration
      const { data: parishes } = await axiosInstance.get('/parish', {
        ...
        // Optional: Add params for filtering or sorting
        params: {
          status: 'active',
          sort: 'name'
        }
      ...
      // Process and transform parishes data
      ...
      // Set parishes with processed data
      ...
      // Ensure parishes are reset on error
      setParishes([]);
    } finally {
...
// };
  ...
  const fetchKoottaymas = async (parishId) => {
    ...
    try {
      ...
      // Fetch koottaymas with comprehensive configuration
      const { data: koottaymas } = await axiosInstance.get(`/koottayma/parish/${parishId}`, {
        ...
        // Optional: Add params for additional filtering
        params: {
          status: 'active'
        }
      });
  
      // Process and transform koottaymas data
      const processedKoottaymas = (koottaymas || []).map(koottayma => ({
        ...koottayma,
        // Add any additional processing
        displayName: koottayma.name,
        shortCode: koottayma.shortCode || 'N/A',
        isActive: koottayma.status === 'active'
      }));
  
      // Sort koottaymas alphabetically
      const sortedKoottaymas = processedKoottaymas.sort((a, b) => 
        a.displayName.localeCompare(b.displayName)
      );
  
      // Set koottaymas with processed data
      setKoottaymas(sortedKoottaymas);
  
      // Log successful fetch
      console.log('Koottaymas Fetched:', {
        parishId,
        totalKoottaymas: sortedKoottaymas.length,
        activeKoottaymas: sortedKoottaymas.filter(k => k.isActive).length
      });
      ...
      console.error('Error fetching koottaymas:', {
        parishId,
        message: error.message,
        status: error.response?.status,
        data: error.response?.data
      ...
      // Ensure koottaymas are reset on error
      setKoottaymas([]);
    } finally {
      // Always turn off loading state
      setLoading(false);
  ...
  const fetchFamilies = async (parishId, koottaymaId) => {
    try {
      ...
      // If no families found, set empty and return
      ...
      // Ensure personsByFamily is an object
      ...
      const familiesWithDetails = families.map(family => {
        ...
        const members = familyPersons.map((person) => person.name).join(', ');
