    ...
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ 
          ...
          {/* <Grid container spacing={3}>
            <Grid item xs={12} lg={12}>
              <Typography variant="h6" gutterBottom>
                Families by Koottayma
              </Typography>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={filteredAndSortedKoottaymaData}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" angle={-45} textAnchor="end" height={80} />
                  <YAxis />
                  <Tooltip />
                  <Bar dataKey="familyCount" fill={theme.palette.primary.main} />
                </BarChart>
              </ResponsiveContainer>
            </Grid>
            ...
            <Grid item xs={12} lg={12}>
              ...
              <ResponsiveContainer width="100%" height={500}>
                <PieChart>
                  ...
                  >
                    {filteredConsolidatedData.map((entry, index) => (
                  ...
                  <tbody>
                    {(koottayma?.families || []).map((family, fIndex) => (
                      <tr key={fIndex}>
                        ...
                        <td style={{ padding: '12px', border: '1px solid #ddd' }}>{family?.phone || 'N/A'}</td>
                        <td style={{ padding: '12px', textAlign: 'right', border: '1px solid #ddd' }}>
                    ...
                    <tr style={{ backgroundColor: theme.palette.grey[100], fontWeight: 600 }}>
                      <td colSpan={3} style={{ padding: '12px', textAlign: 'right', border: '1px solid #ddd' }}>Total:</td>
                      <td style={{ padding: '12px', textAlign: 'right', border: '1px solid #ddd' }}>
        ...
        {/* {allocationData && (
          <StyledPaper sx={{ p: 3, mb: 3 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
              <PieChartIcon sx={{ fontSize: 24, color: theme.palette.secondary.main, mr: 2 }} />
              <Typography variant="h5" sx={{ fontWeight: 700 }}>
                Allocation Reports ({allocationData.year})
              </Typography>
              <Box sx={{ ml: 'auto' }}>
                <Typography variant="h6" color="secondary.main">
                  Total: ₹{allocationData.communitySettings?.total_amount?.toLocaleString()}
                </Typography>
              </Box>
            </Box>
            ...
            <Grid container spacing={3}>
              ...
              {/* <Grid item xs={12} md={4}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
                    <Pie
                      data={allocationData.communitySettings?.settings || []}
                      cx="50%"
                      cy="50%"
                      outerRadius={80}
                      fill="#8884d8"
                      dataKey="allocated_amount"
                      label={({ community_name, percentage }) => `${community_name}: ${percentage}%`}
                    >
                      {(allocationData.communitySettings?.settings || []).map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip formatter={(value) => `₹${value.toLocaleString()}`} />
              ...
              {/* <Grid item xs={12} md={4}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={allocationData.parishAllocations || []}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" angle={-45} textAnchor="end" height={80} />
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="collection" fill={theme.palette.secondary.main} name="Collection" />
                    <Bar dataKey="totalAllocation" fill={theme.palette.primary.main} name="Allocation" />
                  </BarChart>
                </ResponsiveContainer>
              </Grid> */}
              ...
              {/* <Grid item xs={12} md={4}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
                    <Pie
                      data={allocationData.projectSettings?.settings || []}
                      cx="50%"
                      cy="50%"
                      outerRadius={80}
                      fill="#8884d8"
                    ...
                    >
                      {(allocationData.projectSettings?.settings || []).map((entry, index) => (
