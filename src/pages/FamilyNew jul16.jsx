...
const handleSaveCurrentTransactions = async () => {
   ...
   const showTransactionAudit = async (transactionId, personName) => {
    try {
      setAuditLoading(true);
      setSelectedTransactionForAudit({ id: transactionId, personName });
      
      // Fetch audit logs from the API
      const response = await axiosInstance.get(`/transaction/audit/${transactionId}`);
      
      if (response.data && response.data.auditLogs) {
        setTransactionAuditLogs(response.data.auditLogs);
        setAuditDialogOpen(true);
      } else {
        showSnackbar('No audit logs found for this transaction', 'info');
      }
    } catch (error) {
