...
const handlePersonStatusChange = async () => {
  try {
    ...
    const stateSnapshot = {
      ...
      transferredAmount: 0, // Track the exact amount transferred
      transferId: null, // Track the transfer transaction ID
      timestamp: new Date()
    };
...
const updatePersonAndTransactions = async (newHead = null) => {
  try {
    ...
    try {
        ...
        if (transResponse.data && transResponse.data.totalAmount > 0) {
          ...
          if (headId) {
            const transferData = {
              ...
              // Add metadata for easier rollback tracking
              rollbackData: {
      ...
      // 4. Create movement record with transfer information
      const sourceParish = parishes.find(p => p._id === selectedParish);
      const sourceKoottayma = koottaymas.find(k => k.koottaymaId === selectedKoottayma);
      ...
      const movementData = {
        ...
        rollbackData: operations, // Store operations for potential rollback
      ...
      console.error('Error in status update, attempting rollback:', error);
  ...
  try {
    ...
    familyData.forEach(person => {
      ...
      const processedTransaction = {
        ...
        currentYearAmount: person.currentYearAmount || 0,
    ...
    // // Optional: Log insights asynchronously (non-blocking)
