...
const handleEditFamily = async () => {
  try {
    if (!editFamilyData.building || !editFamilyData.phone) {
...
// Add this new function to search family by number:
...
const handleUndoStatusChange = async () => {
  try {
    ...
    try {
      ...
      await axiosInstance.put(`/person/${originalPerson._id}`, {
        status: 'active',
        relation: originalPerson.originalRelation,
        narration: `Status restored from ${statusDetails.status} to active on ${new Date().toLocaleDateString()}`
      });
      ...
      if (originalHead && newHead && originalHead._id !== newHead._id) {
        ...
        await axiosInstance.put(`/person/${originalHead._id}`, {
          relation: 'head',
          status: 'active',
          narration: `Head role restored on ${new Date().toLocaleDateString()}`
        });

        // Revert new head to original relation
        await axiosInstance.put(`/person/${newHead._id}`, {
          relation: newHead.originalRelation,
          status: 'active',
          narration: `Role reverted to ${newHead.originalRelation} on ${new Date().toLocaleDateString()}`
        });
      }

      // 3. CRITICAL: Restore transaction amounts with precision
      if (transferredAmount > 0 && headId) {
        console.log(`Restoring ${transferredAmount} from head ${headId} to person ${originalPerson._id}`);
        
        // Create reverse transfer to restore the original person's transaction
        const restoreTransferData = {
          fromPerson: headId,
          toPerson: originalPerson._id,
          forane: selectedFamily.forane,
          parish: selectedParish,
          family: selectedFamily.id,
          status: 'rollback',
          reason: `Transaction restored due to status rollback`,
          amount: transferredAmount,
          date: new Date().toLocaleDateString('en-GB'),
          originalTransferId: transferId,
          rollbackData: {
            isRollback: true,
            originalStatus: statusDetails.status,
            restoredTo: 'active'
          }
        };

        const restoreResponse = await axiosInstance.post('/transaction/transfer', restoreTransferData);
        console.log('Transaction restoration completed:', restoreResponse.data);
      }
      ...
      const rollbackMovementData = {
        ...
        newRelation: originalPerson.originalRelation,
        movedDate: new Date(),
        status: 'active',
      ...
      showSnackbar(`Successfully reverted all changes. Amount ₹${transferredAmount} restored to ${originalPerson.name}`, 'success');
...
const fetchFamilyData = async (familyId) => {
  ...
  try {
    ...
    const [currentYearResponse, previousYearResponse] = await Promise.allSettled([
      // Current year family data
      axiosInstance.get(`/person/familydata/${familyId}/year/${currentYear}`, {
        timeout: 15000,
        validateStatus: (status) => status >= 200 && status < 300
      }),
      // Previous year family data (single request instead of multiple)
    ...
    // Process all data in a single loop for better performance
    const processedPersons = [];
    const processedTransactions = [];
    
    // Fetch detailed transaction data for each person to filter out transferred transactions
    for (const person of familyData) {
      ...
      const processedPerson = {
        ...
        age: calculateAge(person.dob),
        fullName: `${person.name} ${person.baptismName || ''}`.trim(),
        // Pre-calculate sorting priority
        sortPriority: person.relation === 'head' ? 0 : 1
      };
      processedPersons.push(processedPerson);

      // Fetch detailed current year transactions to filter out transferred ones
      ...
      try {
        ...
        currentYearAmountExcludingTransferred = detailedTransactions
          .filter(transaction => {
            ...
            // Adjust this condition based on how your backend marks transferred transactions
        ...
        // Fallback to the original amount if detailed fetch fails
        currentYearAmountExcludingTransferred = person.currentYearAmount || 0;
      }

      // Process transaction data with filtered current year amount
    ...
    // Optimized sorting using pre-calculated priority
...
const handleMoveFamily = async () => {
  ...
  Cell: ({ row }) => {
    ...
    const getCurrentYearAmountExcludingTransferred = () => {
      ...
      // Get the current year amount from the main transaction data
      // This should already exclude transferred amounts if your backend API is correct
      // If not, you might need to filter out transferred transactions here
